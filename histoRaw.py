...
output_dir = "output/histograms_absolute"
...
for file in os.listdir(fsii_dir):
    ...
    sns.histplot(values_1, bins=30, stat="count", color="darkorange")
    ...
    Image.frombuffer("RGBA", (w, h), canvas1.buffer_rgba(), "raw", "RGBA", 0, 1)\
         .save(os.path.join(output_dir, f"{base_filename}_shapley_only_absolute.png"))
    plt.close(fig1)
    ...
    sns.histplot(values_2, bins=30, stat="count", color="purple")
    plt.title(f"Histogramm – 2nd-Order Interactions\n({base_filename})", fontsize=10)
    plt.xlabel("Wert (absolut)")
    plt.ylabel("Anzahl")
    plt.tight_layout()
    ...
    Image.frombuffer("RGBA", (w, h), canvas2.buffer_rgba(), "raw", "RGBA", 0, 1)\
         .save(os.path.join(output_dir, f"{base_filename}_2nd_only_absolute.png"))
    plt.close(fig2)

    # ===== ORDER 1 + 2 =====
    values_all = np.abs([v for k, v in result.dict_values.items() if len(k) in [1, 2]])
    fig3 = plt.figure(figsize=(6, 4))
    sns.histplot(values_all, bins=30, stat="count", color="royalblue")
    ...
    Image.frombuffer("RGBA", (w, h), canvas3.buffer_rgba(), "raw", "RGBA", 0, 1)\
         .save(os.path.join(output_dir, f"{base_filename}_combined_1_2_absolute.png"))
    plt.close(fig3)

print("\n✅ [DONE] Alle Histogramme mit stat='count' (absolute Häufigkeit) gespeichert.")
